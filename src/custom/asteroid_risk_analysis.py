...
QUERY = """with target_date as (
	select created_at from neo_data_drift order by created_at desc limit 1
	)
select  tn.id,
		tn.min_diameter_km,
		tn.max_diameter_km,
...
from train_neo tn
join prediction_table pt on pt.id::bigint = tn.id
where close_approach_date::date >= (select created_at::date - interval '7 days' from target_date)
  and close_approach_date::date <= (select created_at::date from target_date);"""
...
class AsteroidRiskAnalysis:
    ...
    def compute_physical_metrics(self, df: pd.DataFrame,
                             density: float = 2600.0,
                             diameter_unit: str = 'm',
                             velocity_unit: str = 'm/s') -> pd.DataFrame:
        """
    ...
    def normalize_energy_and_moid(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, MinMaxScaler]:
        """
        ...
        energy = df['impact_energy_j'].replace([np.inf, -np.inf], np.nan).fillna(0.0).astype(float).values.reshape(-1, 1)
        ...
        # Handle NaNs → assign largest distance (lowest risk)
        miss_km = miss_km.fillna(miss_km.max())
        inv_moid = 1.0 / (miss_km.values + EPS)     # <-- use .values to get NumPy array
    ...
    def prepare_probabilities(self, df: pd.DataFrame,
        ...
        classifier must implement predict_proba(X) and feature_cols must be provided.
        """
        df = df.copy()
        if 'probability_being_truely_hazardous' in df.columns:
            df['Phazardous'] = df['probability_being_truely_hazardous'].astype(float).clip(0.0, 1.0)
            return df

        if classifier is None or feature_cols is None:
            raise ValueError("Either provide 'Phazardous' column in df or pass classifier and feature_cols")
    ...
    def compute_weights(self, df_norm: pd.DataFrame,
                    method: str = 'manual',
                    manual_weights: Tuple[float,float,float] = (0.4, 0.4, 0.2),
                    label_col: str = 'is_hazardous') -> Tuple[float,float,float]:
        """
        ...
        if method == 'data-driven':
            if label_col not in df_norm.columns:
                raise ValueError(f"label_col '{label_col}' not found in dataframe for data-driven weighting")
            X = df_norm[['Enorm', 'Rmoid_norm', 'Phazardous']].astype(float).fillna(0.0).values
            y = df_norm[label_col].astype(int).values
            # Use small C to regularize slightly if dataset small
            clf = LogisticRegression(solver='lbfgs', max_iter=1000)
            clf.fit(X, y)
            coefs = clf.coef_.reshape(-1)  # shape (3,)
            # Use absolute value of coefficients to derive importance, then normalize
    ...
    def compute_composite_score(self, df: pd.DataFrame,
        ...
        """
        Apply weights to normalized features and compute composite Risk Score then normalize to [0,1].
        Adds columns: 'RiskScore_raw', 'RiskScorenorm', 'RiskCategory'
        """
        df = df.copy()
        w1, w2, w3 = weights
        df['RiskScore_raw'] = w1 * df['Enorm'] + w2 * df['Rmoid_norm'] + w3 * df['Phazardous']
        # normalize RiskScore_raw to [0,1]
    ...
    def initiate_asteroid_risk_analysis(self, data):
        data_def_1 = self.compute_physical_metrics(df=data, density=2600.0, diameter_unit='km', velocity_unit='km/s')
        data_def_2, scalers = self.normalize_energy_and_moid(df=data_def_1)
        data_def_3 = self.prepare_probabilities(data_def_2)
        manual_w = self.compute_weights(df_norm=data_def_3, method='manual', manual_weights=self.manual_weights)
        dd_w = self.compute_weights(df_norm=data_def_3, method='data-driven', label_col='is_potentially_hazardous')
    ...
    def merge_asteroid_manual_dd_data(self, manual_data, data_dd):
    ...
    def push_final_data_to_db(self, fin_data):
        if fin_data.empty:
            print("The final data is empty...")
            return
        fin_data.to_sql(self.risk_analysis_table_name, engine, if_exists="append", index=False)
        print(f"[INFO] The final data frame is pushed to the database with table name: {self.risk_analysis_table_name}")
...
if __name__=='__main__':
    ...
    print(f"[INFO] Shape of manually weighted data {manual_w_data.shape} and Shape of data driven weighted data {dd_w_data.shape}")
    final_data = risk_analysis.merge_asteroid_manual_dd_data(manual_data=manual_w_data, data_dd=dd_w_data)
    print(f"[INFO] The shape of the final dataframe is {final_data.shape}")
    print(f"[INFO] The columns of the final dataframe is {final_data.columns}")
    risk_analysis.push_final_data_to_db(fin_data=final_data)
    print(f"[INFO] The script work completed")
